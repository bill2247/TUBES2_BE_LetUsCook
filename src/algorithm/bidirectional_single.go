...
func pathTotree(path []Path_element) *data_type.RecipeTree {
	...
	root := &data_type.RecipeTree{Name: scrapping.MapperIdxToName[path[0].Result], Children: nil}
	prev := root
	for i, _ := range path[:len(path)-1] {
		firstRecipe := &data_type.RecipeTree{Name: scrapping.MapperIdxToName[path[i].Idx], Children: nil}
		secondRecipe := &data_type.RecipeTree{Name: scrapping.MapperIdxToName[path[i+1].Result], Children: nil}
